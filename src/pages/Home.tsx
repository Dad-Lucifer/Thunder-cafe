...
const Home = () => {
  ...
  return (
    <div className="min-h-screen bg-black text-white overflow-x-hidden">
      ...
      <section ref={heroRef.ref} className="relative min-h-screen flex items-center justify-center px-4 sm:px-6 lg:px-8">
        {/* Background Effects */}
        ...
        <div className="relative z-10 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          <div 
            className={`space-y-6 text-center lg:text-left ${
              heroRef.isVisible ? 'animate-fade-in-left' : 'opacity-0'
            }`}
          >
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start pt-4 animate-fade-in-up animate-stagger-3">
              <Button 
                asChild
                size="lg"
                className="bg-red-600 hover:bg-red-700 text-white font-bold px-6 sm:px-8 py-3 sm:py-4 text-base sm:text-lg transition-all duration-200 hover:scale-105 hover:shadow-lg hover:shadow-red-600/25 animate-hover-glow"
              >
              ...
              <Button 
                ...
                variant="outline"
                size="lg"
                className="border-red-600 text-red-500 hover:bg-red-600 hover:text-white font-bold px-6 sm:px-8 py-3 sm:py-4 text-base sm:text-lg transition-all duration-200 hover:scale-105 animate-hover-lift"
              >
          ...
          <div 
            className={`relative ${
              heroRef.isVisible ? 'animate-fade-in-right animate-stagger-2' : 'opacity-0'
            }`}
          >
            <div className="relative">
              ...
              {/* Main card */}
              <Card 
                className="relative bg-gray-900/90 border border-red-900/30 rounded-2xl p-4 sm:p-6 lg:p-8 backdrop-blur-sm shadow-2xl animate-hover-lift animate-float cursor-pointer transition-transform hover:scale-105"
                        ...
                        {cafeImages.map((_, index) => (
                          <button
                            ...
                            className={`h-2 w-2 rounded-full transition-colors ${
                              index === currentCafeImage ? 'bg-red-500' : 'bg-white/50'
                            }`}
                            onClick={(e) => {
                              e.stopPropagation();
            ...
            {features.map((feature, index) => (
              <Card 
                ...
                className={`bg-gray-900/80 border border-red-900/20 hover:border-red-600/40 transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-red-600/10 animate-hover-lift ${
                  featuresRef.isVisible ? `animate-fade-in-up animate-stagger-${index + 1}` : 'opacity-0'
                }`}
              >
                ...
                <CardContent className="text-center space-y-4">
                  ...
                  <Button 
                    ...
                    variant="outline"
                    size="sm"
                    className="border-red-600 text-red-500 hover:bg-red-600 hover:text-white transition-all duration-200 animate-hover-glow w-full sm:w-auto"
                  >
                    <Link to={`/booking?game=${feature.slug}`} aria-label={`Book ${feature.title}`}>
                      Book This
                    </Link>
                  </Button>
                </CardContent>
      ...
      <section ref={gamesRef.ref} className="py-16 sm:py-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className={`text-center mb-12 sm:mb-16 ${
            ...
            <p className="text-lg sm:text-xl text-gray-400">
              Fan favorites and competitive titles
            </p>
          </div>
              ...
              >
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant="secondary" className="bg-red-600/20 text-red-400 border-red-600/30 animate-hover-scale text-xs">
                      {game.platform}
                    </Badge>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="flex items-center justify-between">
                    <Button 
                      asChild
                      size="sm"
                      className="bg-red-600 hover:bg-red-700 text-white animate-hover-glow w-full sm:w-auto"
                    >
                      <Link to={`/booking?game=${game.slug}`} aria-label={`Book ${game.title}`}>
                        Book Now
                      </Link>
                    </Button>
                  </div>
          ...
          <div className="text-center mt-8 sm:mt-12">
            <Button 
              ...
              variant="outline"
              size="lg"
              className="border-red-600 text-red-500 hover:bg-red-600 hover:text-white font-bold px-6 sm:px-8 py-3 text-base sm:text-lg transition-all duration-200 hover:scale-105"
            >
      ...
      <section ref={whyRef.ref} className="py-16 sm:py-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-red-950/5 to-black">
        <div className="max-w-7xl mx-auto">
          <div className={`text-center mb-12 sm:mb-16 ${
            whyRef.isVisible ? 'animate-fade-in-up' : 'opacity-0'
          }`}>
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white mb-4">
              Why Choose Us
            </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
            {whyChooseUs.map((item, index) => (
              <div 
                key={item.title}
                className={`text-center space-y-4 animate-hover-lift ${
                  whyRef.isVisible ? `animate-fade-in-up animate-stagger-${index + 1}` : 'opacity-0'
                }`}
              >
                ...
                <p className="text-gray-400 leading-relaxed max-w-sm mx-auto text-sm sm:text-base">
                  {item.description}
                </p>
              </div>
            ))}
          ...
          <div className="text-center mt-12 sm:mt-16">
            <Button 
              asChild
              size="lg"
              className="bg-red-600 hover:bg-red-700 text-white font-bold px-6 sm:px-8 py-3 sm:py-4 text-base sm:text-lg transition-all duration-200 hover:scale-105 hover:shadow-lg hover:shadow-red-600/25"
            >
              <Link to="/booking" aria-label="Book a gaming session">
